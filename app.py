...
MAIL_FROM = os.environ.get('MAIL_FROM', MAIL_USERNAME or 'no-reply@pixoradesign.com')
...
def init_db():
    ...
    db.executescript(
        ...
        CREATE TABLE IF NOT EXISTS blog_posts (
            ...
            content TEXT,
            category TEXT,
            author TEXT DEFAULT 'Pixora Team',
...
def seed_data(db):
    ...
    if not db.execute('SELECT 1 FROM users WHERE email=?', ('admin@pixoradesign.com',)).fetchone():
        db.execute('INSERT INTO users (name,email,password_hash,role) VALUES (?,?,?,?)', ('Admin', 'admin@pixoradesign.com', hash_password('Admin@123'), 'admin'))
    ...
    if db.execute('SELECT COUNT(*) FROM blog_posts').fetchone()[0] == 0:
        posts = [
            ('How Good Packaging Improves Product Sales', 'A practical look at packaging decisions that influence trust and shelf appeal.', 'Strong packaging design communicates value before a customer even picks up the product. At Pixora, we focus on structure, hierarchy, color, and clarity so packaging feels premium and purposeful.', 'Branding', 'Pixora Team', '2026-04-01', '📦'),
            ('What Makes a Brand Identity Memorable', 'Simple identity systems help brands stay recognisable across every touchpoint.', 'A memorable brand identity is not just a logo. It is a consistent visual language that customers recognise in print, digital, packaging, and communication.', 'Design', 'Pixora Team', '2026-03-18', '🎨'),
            ('Why Businesses Need a Better Design Brief', 'A good brief saves time and leads to faster approvals and stronger creative output.', 'A clear design brief reduces revisions, avoids confusion, and helps agencies move quickly with more confidence.', 'Process', 'Pixora Team', '2026-03-02', '📝'),
        ]
...
def create_project_from_quote(quote_id, note='Project created from user start-project request', notify_user=True):
    ...
    db.execute(
        ...
        ''',
        (quote['user_id'], quote_id, title, quote['service'], quote['project_description'], quote['budget_range'], quote['deadline'], 'Pixora Team')
    )
...
def forgot_password():
    ...
    if request.method == 'POST':
        ...
        if not user:
            ...
            email_sent = send_email_notice(email, 'Pixora Password Reset', f'Use this link to reset your password: {reset_link}')
            if email_sent:
...
def admin_create_blog():
    ...
    get_db().execute('INSERT INTO blog_posts (title,excerpt,content,category,author,published_at,cover_emoji,tags,cover_image) VALUES (?,?,?,?,?,?,?,?,?)',
                     (request.form.get('title', '').strip(), request.form.get('excerpt', '').strip(), request.form.get('content', '').strip(), request.form.get('category', '').strip(), request.form.get('author', 'Pixora Team').strip() or 'Pixora Team', request.form.get('published_at', '').strip() or datetime.now().strftime('%Y-%m-%d'), request.form.get('cover_emoji', '📝').strip() or '📝', request.form.get('tags', '').strip(), cover_image))
    get_db().commit()
...
def admin_edit_blog(post_id):
    ...
    if request.method == 'POST':
        ...
        db.execute('UPDATE blog_posts SET title=?, excerpt=?, content=?, category=?, author=?, published_at=?, cover_emoji=?, tags=? WHERE id=?',
                   (request.form.get('title', '').strip(), request.form.get('excerpt', '').strip(), request.form.get('content', '').strip(), request.form.get('category', '').strip(), request.form.get('author', 'Pixora Team').strip() or 'Pixora Team', request.form.get('published_at', '').strip() or datetime.now().strftime('%Y-%m-%d'), request.form.get('cover_emoji', '📝').strip() or '📝', request.form.get('tags', '').strip(), post_id))
        db.commit()
...
def download_invoice_pdf(invoice_id):
    ...
    pdf.cell(0, 12, 'Pixora Design Studio', ln=1)
    ...
    pdf.set_font('Helvetica', '', 11)
    pdf.multi_cell(0, 8, invoice['notes'] or 'Thank you for choosing Pixora Design Studio.')
    pdf.ln(6)
